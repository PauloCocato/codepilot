...
describe('Queue Worker', () => {
  ...
  it('should process valid jobs', async () => {
    ...
    const mockResult = { success: true, attempts: 1, totalCostUsd: 0.05, totalLatencyMs: 5000 };
    ...
    expect(mockProcessFn).toHaveBeenCalledWith('https://github.com/owner/repo/issues/1');
  });
  ...
  it('should reject invalid job data', async () => {
    ...
    expect(capturedProcessor).toBeDefined();
    await expect(capturedProcessor!(mockJob)).rejects.toThrow('Invalid job data');
  });
