...
import { ResolveIssueJobSchema, validateJobData, QUEUE_NAME, JOB_NAMES } from './jobs.js';
...
describe('Queue Jobs', () => {
  ...
  it('should reject job with invalid URL', () => {
    const result = ResolveIssueJobSchema.safeParse({ ...validJob, issueUrl: 'not-a-url' });
    expect(result.success).toBe(false);
  });

  it('should reject job with negative issue number', () => {
    const result = ResolveIssueJobSchema.safeParse({ ...validJob, issueNumber: -1 });
    expect(result.success).toBe(false);
  });

  it('should reject job with invalid triggeredBy', () => {
    const result = ResolveIssueJobSchema.safeParse({ ...validJob, triggeredBy: 'cron' });
    expect(result.success).toBe(false);
  });

  it('should reject empty repoOwner', () => {
    const result = ResolveIssueJobSchema.safeParse({ ...validJob, repoOwner: '' });
    expect(result.success).toBe(false);
  });

  it('should accept all valid triggeredBy values', () => {
    for (const triggeredBy of ['webhook', 'manual', 'api'] as const) {
      const result = ResolveIssueJobSchema.safeParse({ ...validJob, triggeredBy });
      expect(result.success).toBe(true);
    }
  });
