...
describe('ClaudeAdapter', () => {
  describe('complete', () => {
    it('should return a valid CompletionResult on success', async () => {
      const mockClient = createMockClient(vi.fn().mockResolvedValue(makeMockResponse()));
    ...
    it('should map max_tokens stop_reason to length finishReason', async () => {
      const mockClient = createMockClient(
        vi.fn().mockResolvedValue(makeMockResponse({ stop_reason: 'max_tokens' })),
      );
      const adapter = new ClaudeAdapter({ client: mockClient });
    ...
    it('should calculate cost using Sonnet pricing', async () => {
      const mockClient = createMockClient(vi.fn().mockResolvedValue(
        makeMockResponse({ usage: { input_tokens: 1_000_000, output_tokens: 100_000 } }),
      ));
      const adapter = new ClaudeAdapter({ client: mockClient });
  ...
  describe('estimateCost', () => {
    ...
    it('should include systemPrompt in token estimate', () => {
      ...
      expect(withSystem.estimatedInputTokens).toBeGreaterThan(withoutSystem.estimatedInputTokens);
    });
  });
});
