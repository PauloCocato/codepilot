...
import { execute, installDependencies, runTests, applyAndTest } from './executor.js';
...
function createMockDocker(options: MockStreamOptions = {}) {
  ...
  const mockDocker = {
    ...
    modem: {
      demuxStream: vi.fn((_stream: unknown, stdoutWriter: { write: (chunk: Buffer) => void }, stderrWriter: { write: (chunk: Buffer) => void }) => {
...
describe('execute', () => {
  it('should execute a command and return the result', async () => {
    ...
    const result = await execute(sandbox, 'echo hello', undefined, mockDocker as any);
  ...
  it('should return non-zero exit code on failure', async () => {
    ...
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    const result = await execute(sandbox, 'nonexistent', undefined, mockDocker as any);
  ...
  it('should set timedOut and exit code 124 on timeout', async () => {
    const sandbox = createMockSandbox();
    const { mockDocker, mockStream } = createMockDocker({ hang: true, exitCode: 0 });

    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    const result = await execute(sandbox, 'sleep 1000', { timeoutMs: 10 }, mockDocker as any);
...
describe('applyAndTest', () => {
  ...
  it('should install deps and run tests when patch succeeds', async () => {
    ...
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    const result = await applyAndTest(sandbox, 'valid patch', mockDocker as any);
